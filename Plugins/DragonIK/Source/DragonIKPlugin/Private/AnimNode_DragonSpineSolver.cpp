	...
	for (int i = 0; i < NumTransforms-0; i++)
	{
		...
		if (atleast_one_hit == true)
		{
			...
			if (Ignore_Lerping == false)
				Total_spine_alphas[i] = UKismetMathLibrary::FInterpTo(Total_spine_alphas[i], 1, 1 - FMath::Exp(-Smooth_Factor * owning_skel->GetWorld()->GetDeltaSeconds()), Shift_Speed*0.5f);
			else
		...
		else
		{
			...
			if (Ignore_Lerping == false)
				Total_spine_alphas[i] = UKismetMathLibrary::FInterpTo(Total_spine_alphas[i], 0, 1 - FMath::Exp(-Smooth_Factor * owning_skel->GetWorld()->GetDeltaSeconds()), Shift_Speed*0.5f);
			else
			...
			for (int i = 0; i < TraceStartList.Num(); i++)
			{
				...
				if (trace_type == EIKTrace_Type_Plugin::ENUM_LineTrace_Type)
					...
					if (trace_type == EIKTrace_Type_Plugin::ENUM_SphereTrace_Type)
					{
						...
						DrawDebugCapsule(PreviewSkelMeshComp->GetWorld(), TraceStartList[i] - FVector(0, 0, Vector_Difference.Z * 0.5f), Vector_Difference.Size()*0.5f + (Trace_Radius*owner_scale), Trace_Radius*owner_scale, FRotator(0, 0, 0).Quaternion(), FColor::Red, false, 0.1f);
...
//						for (int i = 0; i<spine_Feet_pair[j].Associated_Feet.Num(); i++)
				...
				//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, "Child Bone " + GetChildBone(spine_Feet_pair[i].Associated_Feet[j].BoneName, owning_skel).ToString());
					...
					{
						line_trace_func(*owning_skel
							, spine_AnimatedTransform_pairs[i].Associated_FeetBalls[j].GetLocation() + character_direction_vector *start_const,
							spine_AnimatedTransform_pairs[i].Associated_FeetBalls[j].GetLocation() - character_direction_vector *end_const,
							spine_hit_pairs[i].RV_FeetBalls_Hits[j],
							spine_Feet_pair[i].Associated_Feet_Tips[j].BoneName,
							spine_Feet_pair[i].Associated_Feet_Tips[j].BoneName
							, spine_hit_pairs[i].RV_FeetBalls_Hits[j], FLinearColor::Red,false);
				...
				//				FVector Feet_Offseted_Location_Animated = spine_AnimatedTransform_pairs[i].Associated_Feet[j].GetLocation() + (UKismetMathLibrary::TransformDirection(owning_skel->GetComponentToWorld(), spine_Feet_pair[i].feet_trace_offset[j]));


					line_trace_func(*owning_skel
						, spine_AnimatedTransform_pairs[i].Associated_Feet[j].GetLocation() + character_direction_vector *start_const,
						spine_AnimatedTransform_pairs[i].Associated_Feet[j].GetLocation() - character_direction_vector *end_const,
						spine_hit_pairs[i].RV_Feet_Hits[j],
						spine_Feet_pair[i].Associated_Feet[j].BoneName,
						spine_Feet_pair[i].Associated_Feet[j].BoneName
			...
			//	FVector Feet_Offseted_Location_Rest = spine_Transform_pairs[i].Associated_Feet[j].GetLocation() + (UKismetMathLibrary::TransformDirection(owning_skel->GetComponentToWorld(), spine_Feet_pair[i].feet_trace_offset[j]));
		...
		//	if (i == 0)
			{
				...
				if (SolverReferencePose == ERefPosePluginEnum::VE_Animated)
				{
					...
					//FVector forward_dir = UKismetMathLibrary::Cross_VectorVector(character_direction_vector, Forward_Direction_Vector);
					...
					//FVector right_dir = UKismetMathLibrary::Cross_VectorVector(character_direction_vector, UKismetMathLibrary::GetUpVector(character_direction_vector.Rotation()));
				...
				//	if (every_foot_dont_have_child == false)
					...
					line_trace_func(*owning_skel, feet_centre_point + f_offset_i + character_direction_vector*(line_trace_upper_height*component_scale) + (right_dir * component_scale * virtual_leg_width), feet_centre_point - character_direction_vector * (line_trace_downward_height * 1 * component_scale) + (right_dir * component_scale * virtual_leg_width), spine_hit_pairs[i].Parent_Left_Hit, spine_Feet_pair[i].Spine_Involved.BoneName, spine_Feet_pair[i].Spine_Involved.BoneName, spine_hit_pairs[i].Parent_Left_Hit, FLinearColor::Yellow, true);
					line_trace_func(*owning_skel, feet_centre_point + f_offset_i + character_direction_vector * (line_trace_upper_height*component_scale) - (right_dir * component_scale * virtual_leg_width), feet_centre_point - character_direction_vector * (line_trace_downward_height * 1 * component_scale) - (right_dir * component_scale * virtual_leg_width), spine_hit_pairs[i].Parent_Right_Hit, spine_Feet_pair[i].Spine_Involved.BoneName, spine_Feet_pair[i].Spine_Involved.BoneName, spine_hit_pairs[i].Parent_Right_Hit, FLinearColor::Yellow, true);



					line_trace_func(*owning_skel, feet_centre_point + f_offset_i + character_direction_vector * (line_trace_upper_height*component_scale) + (forward_dir * component_scale * virtual_leg_width), feet_centre_point - character_direction_vector * (line_trace_downward_height * 1 * component_scale) + (forward_dir * component_scale * virtual_leg_width), spine_hit_pairs[i].Parent_Front_Hit, spine_Feet_pair[i].Spine_Involved.BoneName, spine_Feet_pair[i].Spine_Involved.BoneName, spine_hit_pairs[i].Parent_Front_Hit, FLinearColor::Green, true);
					line_trace_func(*owning_skel, feet_centre_point + f_offset_i + character_direction_vector * (line_trace_upper_height*component_scale) - (forward_dir * component_scale * virtual_leg_width), feet_centre_point - character_direction_vector * (line_trace_downward_height * 1 * component_scale) - (forward_dir * component_scale * virtual_leg_width), spine_hit_pairs[i].Parent_Back_Hit, spine_Feet_pair[i].Spine_Involved.BoneName, spine_Feet_pair[i].Spine_Involved.BoneName, spine_hit_pairs[i].Parent_Back_Hit, FLinearColor::Green, true);
...
TArray<FName> FAnimNode_DragonSpineSolver::BoneArrayMachine(int32 index, FName starting, FName ending, bool is_foot)
{
	...
	do
	{
		...
		if (is_foot)
		{
			if (Check_Loop_Exist(dragon_input_data.FeetBones[index].Feet_Trace_Offset,dragon_input_data.FeetBones[index].Feet_Heights,starting, spine_bones[spine_bones.Num() - 1], Total_spine_bones))
			{
				return spine_bones;
			}
		}
...
bool FAnimNode_DragonSpineSolver::Check_Loop_Exist(FVector feet_trace_offset,float feet_height,FName start_bone, FName input_bone, TArray<FName>& total_spine_bones)
{
...
//	2,3,4,5,6
	...
	for (int i = 0; i < combined_indices.Num(); i++)
	{
		...
		if (i > 0 && i < combined_indices.Num()-1)
		{
			...
			FVector new_location = (spine_hit_between[i-1].ImpactPoint + character_direction_vector_CS *(spine_between_heights[i-1]   ));
...
void FAnimNode_DragonSpineSolver::Snake_ImpactRotation(int origin_point_index, FTransform &OutputTransform, FCSPose<FCompactPose>& MeshBases)
{
	...
	FVector new_location = owning_skel->GetComponentTransform().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Spine_Hit.ImpactPoint + (character_direction_vector_CS*Total_spine_heights[origin_point_index]) );
	...
	FVector pointing_to_transformpoint = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Spine_Hit.ImpactPoint + (character_direction_vector_CS*Total_spine_heights[origin_point_index] ));

	FRotator position_Based_rot = final_rot;

	FVector Feet_Mid_Point = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Spine_Hit.ImpactPoint);
		...
		if (owning_skel->GetOwner())
		{
			...
			//					Intensifier_forward = FMath::Lerp<float>(Intensifier_forward, 0, FMath::Clamp<float>((spine_median_result / (10 * Slope_Detection_Strength)), 0, 1));
				...
				if (spine_hit_pairs[origin_point_index].Parent_Front_Hit.bBlockingHit&&spine_hit_pairs[origin_point_index].Parent_Back_Hit.bBlockingHit)
					Intensifier_forward = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Hit.ImpactPoint).Z)* Pelvis_ForwardRotation_Intensity_INPUT;
			...
			//forward_direction = UKismetMathLibrary::InverseTransformDirection(owning_skel->GetComponentToWorld(),owning_skel->GetForwardVector()) * Intensifier_forward;

			//	forward_direction = UKismetMathLibrary::TransformDirection(owning_skel->GetComponentToWorld(), FVector(0,1,0))*FMath::Abs(Intensifier_forward);
			...
			//	forward_direction = FVector(0,0,0);
			...
			//					Intensifier_side = (spine_hit_pairs[origin_point_index].RV_Feet_Hits[0].ImpactPoint.Z - spine_hit_pairs[origin_point_index].RV_Feet_Hits[1].ImpactPoint.Z)*direction* Body_Rotation_Intensity *-1;
				...
				if (spine_hit_pairs[origin_point_index].Parent_Left_Hit.bBlockingHit&&spine_hit_pairs[origin_point_index].Parent_Right_Hit.bBlockingHit)
					Intensifier_side = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Right_Hit.ImpactPoint).Z)* Body_Rotation_Intensity*0.5f;
				else
					Intensifier_side = 0;
	...
	else if (origin_point_index == spine_Transform_pairs.Num() - 1)
	{
		...
		if (owning_skel->GetOwner())
		{
			...
			//					forward_intensity = FMath::Lerp<float>(forward_intensity, 0, FMath::Clamp<float>((spine_median_result / (10 * Slope_Detection_Strength)), 0, 1));
				...
				if (spine_hit_pairs[origin_point_index].Parent_Front_Hit.bBlockingHit&&spine_hit_pairs[origin_point_index].Parent_Back_Hit.bBlockingHit)
					forward_intensity = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Hit.ImpactPoint).Z)* Chest_ForwardRotation_Intensity_INPUT;
				else
					forward_intensity = 0;
			...
			if (spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() > 2 || Use_Fake_Chest_Rotations || spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() == 0)
			{
				...
				if (spine_hit_pairs[origin_point_index].Parent_Left_Hit.bBlockingHit&&spine_hit_pairs[origin_point_index].Parent_Right_Hit.bBlockingHit)
					right_intensity = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Right_Hit.ImpactPoint).Z)* Body_Rotation_Intensity*0.5f;
				else
					right_intensity = 0;
			...
			else
			{
				...
				right_intensity = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].RV_Feet_Hits[0].ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].RV_Feet_Hits[1].ImpactPoint).Z)*direction* Body_Rotation_Intensity *-1 * 0.5f;
	...
	//	if (Z_Offset == 10000000000)
		{
			if(spine_hit_pairs[origin_point_index].Parent_Spine_Hit.bBlockingHit)
			 Z_Offset = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Spine_Hit.ImpactPoint);
			else
...
//				else
				{
					if (Ignore_Lerping == false)
					OutputTransform.SetLocation(UKismetMathLibrary::VInterpTo(OutputTransform.GetLocation(),  Z_Offset + character_direction_vector_CS * Total_spine_heights[origin_point_index] * component_scale, 1 - FMath::Exp(-Smooth_Factor * owning_skel->GetWorld()->GetDeltaSeconds()), Location_Lerp_Speed* 0.05f));
					else
						OutputTransform.SetLocation( Z_Offset + character_direction_vector_CS * Total_spine_heights[origin_point_index] * component_scale);
...
void FAnimNode_DragonSpineSolver::Dragon_ImpactRotation(int origin_point_index, FTransform &OutputTransform, FCSPose<FCompactPose>& MeshBases, bool is_reverse)
{
	...
	float pevlis_rot_temp = Pelvis_ForwardRotation_Intensity *0.5;
	...
	if (spine_hit_pairs.Num() > origin_point_index - 1)
	{
		...
		//	final_rot = FRotator((YZ_rot.Pitch), final_bone_rotation.Yaw, XZ_rot.Roll);
			...
			*/
			...
			FVector parent_spine_hit_CS = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Spine_Hit.ImpactPoint);
...
//				float Z_Offset = 10000000000;
				...
				if (Use_FeetTips == false)
				{
					...
					for (int32 i = 0; i < spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num(); i++)
					{
						...
						inv_transform.SetLocation(spine_hit_pairs[origin_point_index].RV_Feet_Hits[i].ImpactPoint);
						...
						{
							//Z_Offset = spine_hit_pairs[origin_point_index].RV_Feet_Hits[i].ImpactPoint;
				...
				else
				{
					for (int32 i = 0; i < spine_hit_pairs[origin_point_index].RV_FeetBalls_Hits.Num(); i++)
					{
						...
						inv_transform.SetLocation(spine_hit_pairs[origin_point_index].RV_FeetBalls_Hits[i].ImpactPoint);
				...
				Z_Offset =FVector(Z_Offset.X, Z_Offset.Y,FMath::Clamp(Z_Offset.Z, owning_skel->GetComponentToWorld().InverseTransformPosition(owning_skel->GetComponentLocation()).Z - Maximum_Dip_Height, owning_skel->GetComponentToWorld().InverseTransformPosition(owning_skel->GetComponentLocation()).Z+ Maximum_Dip_Height));

//				Z_Offset = character_direction_vector*FMath::Clamp(Z_Offset.Z, owning_skel->GetComponentLocation().Z - Maximum_Dip_Height * component_scale, owning_skel->GetComponentLocation().Z);
			...
			if (spine_hit_pairs[opposite_index].RV_Feet_Hits.Num() == 2)
			{
				...
				if (spine_hit_pairs[opposite_index].RV_Feet_Hits[0].ImpactPoint.Z > spine_hit_pairs[opposite_index].RV_Feet_Hits[1].ImpactPoint.Z)
					feet_difference_offset_opp = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[opposite_index].RV_Feet_Hits[1].ImpactPoint).Z;
				else
					feet_difference_offset_opp = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[opposite_index].RV_Feet_Hits[0].ImpactPoint).Z;
			...
			Feet_Opposite_Mid_Point = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[opposite_index].Parent_Spine_Hit.ImpactPoint);
		...
		//	FVector feet_opp_world = owning_skel->GetComponentToWorld().TransformPosition(Feet_Opposite_Mid_Point);
			...
			float AC_Cross_Value = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Hit.ImpactPoint).Z);
		...
		//		Slanted_Height_Offset = 
			...
			if (atleast_one_hit)
			{
				...
				if (origin_point_index == 0)
				{
					...
					if (Use_Fake_Pelvis_Rotations == false)
					{
					//	new_location += character_direction_vector * (OG_Cross_Value*Slanted_Height_Offset*FMath::Clamp<float>((spine_median_result / 10)*slop_tolerance, 0, 1));


						new_location += character_direction_vector_CS * ((-OG_Cross_Value)*Slanted_Height_Offset*FMath::Clamp<float>((spine_median_result / 10)*slop_tolerance, 0, 1));

					//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, "slanted pelvis value " + FString::SanitizeFloat(Feet_Mid_Point.Z - Feet_Opposite_Mid_Point.Z));
					...
					//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, "slanted pelvis AC_Cross_Value " + FString::SanitizeFloat(AC_Cross_Value));
				...
				if (spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num()==2)
				{
					...
					/*
					if(spine_hit_pairs[origin_point_index].RV_Feet_Hits[0].ImpactPoint.Z > spine_hit_pairs[origin_point_index].RV_Feet_Hits[1].ImpactPoint.Z)
					individual_feet_diff = spine_hit_pairs[origin_point_index].RV_Feet_Hits[0].ImpactPoint.Z - spine_hit_pairs[origin_point_index].RV_Feet_Hits[1].ImpactPoint.Z;
					else
						individual_feet_diff = spine_hit_pairs[origin_point_index].RV_Feet_Hits[1].ImpactPoint.Z - spine_hit_pairs[origin_point_index].RV_Feet_Hits[0].ImpactPoint.Z;
						*/


					FVector feet_0_impact_cs = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].RV_Feet_Hits[0].ImpactPoint);

					FVector feet_1_impact_cs = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].RV_Feet_Hits[1].ImpactPoint);

					//owning_skel->GetComponentToWorld().InverseTransformPosition

					if ( (origin_point_index==0 && Use_Fake_Pelvis_Rotations) || (origin_point_index > 0 && Use_Fake_Chest_Rotations))
					{
						feet_0_impact_cs = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Right_Hit.ImpactPoint);
						feet_1_impact_cs = owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Hit.ImpactPoint);
					}
...
//						Intensifier_side = FMath::Clamp<float>((Feet_Mid_Point.Z - (Feet_Opposite_Mid_Point.Z)) * Pelvis_ForwardRotation_Intensity, -10000, 10000)  *-1;
						...
						Intensifier_side = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].RV_Feet_Hits[0].ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].RV_Feet_Hits[1].ImpactPoint).Z)*direction* Body_Rotation_Intensity *-1 * 0.5f;
...
//					forward_intensity = FMath::Lerp<float>(forward_intensity, 0, FMath::Clamp<float>((spine_median_result / (10 * Slope_Detection_Strength)), 0, 1));
					...
					if (spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num()>2 || Use_Fake_Chest_Rotations || spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() == 0)
					{
						...
						if (spine_hit_pairs[origin_point_index].Parent_Front_Hit.bBlockingHit&&spine_hit_pairs[origin_point_index].Parent_Back_Hit.bBlockingHit)
							forward_intensity = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Hit.ImpactPoint).Z)* Chest_ForwardRotation_Intensity;
						else
							forward_intensity = 0;
...
//				FVector right_dir_vector = FVector::CrossProduct(forward_dir_forward, character_direction_vector);
				...
				FVector roll_based_location = RotateAroundPoint(spine_position, right_dir_vector*-1, spine_hit_pairs[origin_point_index].Parent_Spine_Hit.ImpactPoint, OutputTransform.Rotator().Roll);

				FVector pitch_based_location = RotateAroundPoint(spine_position, forward_dir_forward*-1, spine_hit_pairs[origin_point_index].Parent_Spine_Hit.ImpactPoint, OutputTransform.Rotator().Pitch);
			...
			//		final_transformation.SetLocation(roll_based_location);
				...
				if (Rotate_Around_Translate)
				{
					if (!Use_Fake_Pelvis_Rotations && !Use_Fake_Chest_Rotations)
						final_transformation.SetLocation(roll_based_location);
				}
				
				//final_transformation.SetLocation(UKismetMathLibrary::VLerp(pitch_based_location,roll_based_location , FMath::Clamp<float> ( ( FMath::Abs(OutputTransform.Rotator().Roll) - FMath::Abs(OutputTransform.Rotator().Pitch))*0.1f, 0, 1)));
...
//				FVector pitch_based_location = RotateAroundPoint(final_transformation.GetLocation(), right_dir_vector, spine_hit_pairs[origin_point_index].Parent_Spine_Hit.ImpactPoint, OutputTransform.Rotator().Roll*-1);

//				FVector pitch_based_location = RotateAroundPoint(final_transformation.GetLocation(), FVector(1,0,0), spine_hit_pairs[origin_point_index].Parent_Spine_Hit.ImpactPoint, OutputTransform.Rotator().Roll);

//				final_transformation.SetLocation(pitch_based_location);



			//	final_transformation.SetLocation(spine_hit_pairs[origin_point_index].Parent_Spine_Hit.ImpactPoint);
			...
			//	final_transformation = owning_skel->GetComponentToWorld()*final_transformation;
				...
				if (atleast_one_hit)
				{
					...
					//	final_transformation.SetLocation(spine_hit_pairs[origin_point_index].Parent_Spine_Hit.ImpactPoint+Total_spine_heights[origin_point_index] );
						...
						if(Ignore_Lerping==false)
					    OutputTransform.SetLocation(UKismetMathLibrary::VInterpTo(FVector(final_transformation.GetLocation().X, final_transformation.GetLocation().Y, OutputTransform.GetLocation().Z), final_transformation.GetLocation() + offset_worldspace, 1 - FMath::Exp(-Smooth_Factor * owning_skel->GetWorld()->GetDeltaSeconds()), Location_Lerp_Speed* 0.05f));
						else
						OutputTransform.SetLocation(final_transformation.GetLocation() + offset_worldspace);
				...
				else
				{
					...
					{
						if (Ignore_Lerping == false)
						OutputTransform.SetLocation(UKismetMathLibrary::VInterpTo(FVector(OutputTransform.GetLocation()), location_reset, 1 - FMath::Exp(-Smooth_Factor * owning_skel->GetWorld()->GetDeltaSeconds()), Location_Lerp_Speed* 0.05f));
						else
						{
							OutputTransform.SetLocation(location_reset);
						}
		...
		//	else
			{
		//		if (Ignore_Lerping == false)
		//			OutputTransform.SetLocation(UKismetMathLibrary::VInterpTo(FVector(OutputTransform.GetLocation().X, OutputTransform.GetLocation().Y, OutputTransform.GetLocation().Z), location_reset, 1 - FMath::Exp(-Smooth_Factor * owning_skel->GetWorld()->GetDeltaSeconds()), Location_Lerp_Speed* 0.05f));
		//		else
		//			OutputTransform.SetLocation(location_reset);
			}
