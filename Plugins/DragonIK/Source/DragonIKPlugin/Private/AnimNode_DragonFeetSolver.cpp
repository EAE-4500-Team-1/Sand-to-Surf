		...
		for (int feet_index = 0; feet_index < spine_hit_pairs[spine_index].RV_Feet_Hits.Num(); feet_index++)
		{
			...
			if (spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].bBlockingHit)
			{
				...
				if (spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].bBlockingHit&& atleast_one_hit)
				{
					...
					float feet_limit = FMath::Abs(spine_Feet_pair[spine_index].feet_rotation_limit[feet_index]);
...
void FAnimNode_DragonFeetSolver::GetFeetHeights(FComponentSpacePoseContext & Output)
{
	...
	for (int i = 0; i < spine_Feet_pair.Num(); i++)
	{
		for (int j = 0; j < spine_Feet_pair[i].Associated_Feet.Num(); j++)
		{
			...
			FeetRootHeights[i].Add(FMath::Abs(bonetraceTransform.GetLocation().Z - FVector(0, 0, 0).Z)*owning_skel->GetComponentScale().Z + (spine_Feet_pair[i].Feet_Heights[j] * owning_skel->GetComponentScale().Z));
			else
	...
	for (int32 i = 0; i < spine_hit_pairs.Num(); i++)
	{
		...
		//if (spine_Transform_pairs[i].Associated_Feet.Num() > 0 && spine_hit_pairs[i].RV_Feet_Hits.Num() > 0)
		{
			for (int32 j = 0; j < spine_Feet_pair[i].Associated_Feet.Num(); j++)
			{
				...
				offseted_linetrace_location += (UKismetMathLibrary::TransformDirection(owning_skel->GetComponentToWorld(), spine_Feet_pair[i].feet_trace_offset[j])) ;
				...
				*/
				...
				float end_const = FeetRootHeights[i][j] + line_trace_down_height;
...
*/
				...
				if (( owning_skel->GetComponentToWorld().InverseTransformPosition( spine_hit_pairs[k].RV_Feet_Hits[i].ImpactPoint).Z - (FVector(0,0,0).Z)) > Vertical_Solver_downward_Scale)
				{
					atleast_one_hit = false;
				}
...
FVector FAnimNode_DragonFeetSolver::AnimLocLerp(FVector start_pos, FVector end_pos,float delta_seconds)
{
	...
	FVector vector_difference = (start_pos_val - end_pos_val)/FMath::Clamp<float>(5-Location_Lerp_Speed,1,10);
...
//	FQuat rotation_difference = (start_pos_val - end_pos_val)*(1/5);
	...
	FQuat rotation_difference = ( (start_pos_val.Inverse()*(end_pos_val))).GetNormalized();
...
bool FAnimNode_DragonFeetSolver::Check_Loop_Exist(FVector feet_trace_offset,FVector knee_direction_offset,float feet_rotation_limit,FRotator feet_rotation_offset,float feet_height, FName start_bone,FName knee_bone,FName thigh_bone, FName input_bone, TArray<FName> total_spine_bones)
{
